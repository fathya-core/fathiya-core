...
8. From the `الموصلات` card, launch one configured connector with
   `تشغيل كمهمة`. Confirm it queues a normal task, runs `connector_profile`,
   and records the connector result in the progress log and receipt.
9. In the integrations panel, select `ربط Zapier MCP محليًا`, complete OAuth,
   then confirm `fathiya-runtime zapier-actions --refresh` returns the live app
   and action counts without printing credentials.
10. In `مشغّل Zapier المقروء`, select one listed read action. Confirm it queues a
   normal task with a `Zapier action: <app> / <action>` prompt and preserves the
   normal approval/progress/receipt flow.
11. From the loopback `/agent-tasks` page, open the local setup action for
...
12. Use the integration probe button for OpenRouter, Supabase, n8n, Zapier MCP,
...
13. Use `تشغيل وكيل` from an integration card, then confirm a normal task is
    queued, `integration_probe` runs through the worker, and the result appears
    in the progress log and receipts panel.
14. Use `تشغيل شبكة الوكلاء` from `/agent-tasks`. Confirm the queued task runs
...
15. From the completed mesh-audit result, use one `next_actions` item. Confirm
...
16. Submit a read-only exact action such as:
...
17. Submit a Zapier write action and confirm it remains in
    `awaiting_approval` until the operator approves it.
18. Submit `اعرض الموصلات ونفّذ الفحوصات الجاهزة`, then confirm the task shows
...
19. Confirm a sensitive follow-up stores an execution checkpoint and resumes
    after approval without replaying completed rounds.
20. Submit a broad local-tool task and confirm `local_capability_inventory`
    reports live readiness for the local execution mesh.
21. Submit an `agent_delegate` request and confirm it stops at
    `awaiting_approval` before invoking Claude Code, Cursor, or Manus.
22. In Kali WSL, run `cursor-agent status`. The runtime treats an installed but
...
23. Submit `افحص جاهزية حساب التداول التجريبي`. Confirm Binance Spot Testnet
...
24. Restart the local control plane and confirm the primary paper-trading agent
...
25. Place a new report in the continuous knowledge inbox. Confirm the watcher
