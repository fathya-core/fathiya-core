...
function makeReceiptId(tool: string): string {
  ...
  const slug = tool.replace(/[^a-z0-9]/gi, '').slice(0, 6).toUpperCase();
...
function ok(
  tool: string,
  data: unknown,
  opts?: { quality_warnings?: string[]; requires_approval?: boolean; quality_gate_passed?: boolean }
): MCPToolResult {
  return {
    success: true,
    tool,
...
export function tool_knowledge_get_awareness_state(
...
export function tool_knowledge_search(
  index: { cards?: Array<{ id: string; title: string; category: string; tags: string[]; summary: string; type: string }> },
...
export function tool_intake_submit_raw(
  params: { content?: string; source?: string; category?: string }
): MCPToolResult {
  const content = (params.content ?? '').trim();
  const source = params.source ?? 'zapier';

  if (!content) return fail('intake_submit_raw', 'content param is required.');
  if (content.length > 50_000) return fail('intake_submit_raw', 'content exceeds 50,000 chars limit.');
...
export function tool_crypto_create_signal_card(
  params: {
    source?: string;
    asset?: string;
    sector?: string;
  ...
  return ok('crypto_create_signal_card', {
    card,
...
export function tool_queue_get_pending(
  runtimeQueue: { items?: Array<Record<string, unknown>> }
): MCPToolResult {
...
export function tool_quality_gate_check(
  params: { text?: string }
): MCPToolResult {
